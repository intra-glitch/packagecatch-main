                        ...
                        {(activeTab === 'products' || activeTab === 'drafts') && (
                            <div>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '20px' }}>
                                    {products.map(p => (
                                        <div key={p.id} style={{ background: '#fff', borderRadius: '24px', border: '1px solid #f0f0f0', padding: '16px', display: 'flex', gap: '16px', alignItems: 'center' }}>
                                            ...
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '6px' }}>
                                                    <button onClick={() => navigate(`/admin/products/edit/${p.id}`)} style={{ padding: '6px 12px', borderRadius: '6px', background: '#f5f5f5', border: 'none', fontWeight: '800', fontSize: '10px', cursor: 'pointer' }}>EDIT</button>
                                                    {activeTab === 'drafts' && <button onClick={() => { productService.update(p.id, { ...p, status: 'active' }).then(() => loadData()) }} style={{ padding: '6px 12px', borderRadius: '6px', background: '#2a9d8f', color: '#fff', border: 'none', fontWeight: '800', fontSize: '10px', cursor: 'pointer' }}>PUBLISH</button>}
                                                </div>
                                            </div>
                                        </div>
                                    ))}
